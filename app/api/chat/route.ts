...
import { getAwsIconStyleInfo, listAllAwsServices, getServiceNameReference } from "@/lib/aws-icon-styles"
import {
    generateRequestId,
    logRequestStart,
    logToolCall,
...
async function handleChatRequest(req: Request): Promise<Response> {
    ...
    logRequestStart(chatRequestId, {
        userId,
        sessionId: validSessionId,
        modelId,
        userMessage: typeof lastUserMessage?.content === "string"
            ? lastUserMessage.content.substring(0, 500)
            : lastUserMessage?.parts?.find((p: any) => p.type === "text")?.text?.substring(0, 500),
...
"""`
    ...
    const result = streamText({
        ...
        onFinish: ({ text, totalUsage, reasoning }) => {
            ...
            // Log the response including reasoning
            logResponse(chatRequestId, {
                responseText: text,
...
Notes:
- For AWS diagrams, ALWAYS call get_aws_icon_style first to get correct colors, then use **AWS Architecture Icons 2026 (aws4)** with width="60" height="60" for service icons.
...
Call this tool to get shape names and usage syntax for a specific library.`,
            ...
            get_aws_icon_style: {
                description: `Get the correct draw.io style for AWS architecture icons (aws4 library) with official AWS colors.

Returns the exact fillColor, shape, and style string for AWS service icons and group containers.
Use this BEFORE creating AWS architecture diagrams to ensure icons have the correct official AWS colors.

Supports:
- AWS service icons (e.g., "s3", "lambda", "bedrock", "ec2", "rds")
- AWS group containers (e.g., "group_aws_cloud", "group_vpc", "group_subnet_public", "group_account")
- Common aliases are auto-resolved (e.g., "acm"→certificate_manager, "alb"→application_load_balancer, "iam"→identity_and_access_management, "kms"→key_management_service, "vpc_endpoint"→endpoints, "igw"→internet_gateway, "ebs"→elastic_block_store, "efs"→elastic_file_system)
- Pass "list" as the only service to get all available services and groups
- Pass "reference" to get a compact name reference list

All AWS service icons should use width="60" height="60". Groups use variable sizes.`,
                inputSchema: z.object({
                    services: z
                        .array(z.string())
                        .describe(
                            "Array of AWS service names (e.g., ['s3', 'lambda', 'bedrock', 'group_vpc']). Use ['list'] to see all available services.",
                        ),
                }),
                execute: async ({ services }) => {
                    const startTime = Date.now()
                    let result: string
                    if (services.length === 1 && services[0].toLowerCase() === "list") {
                        result = listAllAwsServices()
                    } else if (services.length === 1 && services[0].toLowerCase() === "reference") {
                        result = getServiceNameReference()
                    } else {
                        result = getAwsIconStyleInfo(services)
                    }
                    const durationMs = Date.now() - startTime
                    logToolCall(chatRequestId, "get_aws_icon_style", { services }, result, durationMs)
                    return result
                },
            },
        },
